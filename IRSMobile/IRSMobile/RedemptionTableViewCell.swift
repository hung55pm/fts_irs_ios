...
class RedemptionTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        labdate.autoSetDimension(.width, toSize: screenSize.width/6)
        labseries.autoSetDimension(.width, toSize: screenSize.width/6)
        labunit.autoSetDimension(.width, toSize: (3*screenSize.width)/20)
        labprice.autoSetDimension(.width, toSize: (3*screenSize.width)/20)
        labamount.autoSetDimension(.width, toSize: screenSize.width/6)
        setupview()
    }
    ...
    func setupview() {
        ...
        addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[v1]-0-|", options: NSLayoutFormatOptions(), metrics: nil, views: ["v1" : labseries]))
