...
class RedemCashViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    override func viewDidLoad() {
        ...
        tableview.register(RemCashTableViewCell.self, forCellReuseIdentifier: "sub")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.setNavigationBarItem(title: "Confirmation of Cash Paid")
        self.setNavigationBar(title: "Confirmation of Cash Paid")
   ...
   /* func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cells = Bundle.main.loadNibNamed("RemCashTableViewCell", owner: self, options: nil)?.first as! RemCashTableViewCell
        ...
        cells.txt_date_paid.text = formats.formatdatetoddMMMyyyy(str: array[indexPath.row ].DATE_PAID!)
        ...
        connect.getredemptionCash(investorID: investorId, stratdate: start, enddate: end, completionHandler: {(result) in
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                if((result?.count)!>0){
                    ...
                    var arrayview = self.control_until.gettotalrem(arr: self.array)
